...
%%%% music/parts/instrument_library/resources/books/strings/book_violoncello.ily
%%%% version: 4.0
...
\book {  
  ...
  \bookpart {
    ...
    % Score 
      ...
      \score { \scoring_violoncello_part \layout { %{ #(layout-set-staff-size 20) %} } \midi { \set Score.midiChannelMapping = #'instrument } }

    % Articulated MIDI score block %
      % (Purposely creates no visual output) %
      \score { \unfoldRepeats \articulate << \scoring_violoncello_part >> \midi { \set Score.midiChannelMapping = #'instrument } }
  }
}